# Bài thực hành: Băm dữ liệu và kiểm tra tính toàn vẹn bằng SHA-256/SHA-512
...
| File | Mô tả |
|---|---|
| `bai1_bam_du_lieu_sha256_sha512.py` | Băm dữ liệu dạng chuỗi bằng SHA-256 và SHA-512, sau đó so sánh dữ liệu ban đầu với dữ liệu đã sửa. |
| `bai2_kiem_tra_anh_sha512.py` | Tính mã băm SHA-512 của file ảnh `practice_note.png` để kiểm tra file có bị thay đổi hay không. |
| `bai3_gui_nhan_file_sha512.py` | Mô phỏng quá trình gửi/nhận file và xác thực tính toàn vẹn bằng SHA-512. |
...
```bash
git commit -m "Initial commit: SHA hash integrity practice"
```

### Bước 4: Kết nối repository GitHub

Thay `YOUR_USERNAME` và `YOUR_REPOSITORY` bằng thông tin repository của bạn:
...
File `received_practice_note.png` sẽ được tạo tự động khi chạy bài 3, vì vậy không cần đưa file này lên GitHub.
